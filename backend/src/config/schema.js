...
const ensureSchema = async () => {
  console.log('Starting schema setup with retry logic...');
  
  try {
    // Try to create tables with timeout
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Schema setup timeout - likely database connectivity issue')), 15000)
    );
    ...
    console.warn('Server will start in degraded mode without full database initialization');
